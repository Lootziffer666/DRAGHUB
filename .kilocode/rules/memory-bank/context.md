...
- **Correction note (kept for history)**: an earlier session summary claimed M9 (Dock) and M11 (Start menu) were blocked on M8, echoing PLAN.md's own "prerequisite for M9–M11" claim without checking it. The maintainer pushed back, correctly — the Dock's quick-switch just calls the existing `openRepo(fullName)` and its per-pin badge counts are independent background fetches; M11's Codespaces/Releases/Wiki are all scoped to the single currently-open repo. Both were built before M8.
- [x] **PLAN.md M8 — Multi-repo workspaces refactor** (done last, after everything else, so its blast radius could be verified against all finished features): `store.tsx`'s reducer state is now `repos: Record<"owner/repo", RepoState>` + `activeRepoKey`, with `RepoState` exactly as sketched in the plan. The plan's mitigation was applied inverted: instead of migrating every consumer to a `useActiveRepo()` selector, the provider **projects the active repo back into the unchanged legacy `state` shape** (`StateView`) — Explorer, FileView, Tabs, AddressBar, StatusBar and all feature modules didn't need touching. Repo-scoped actions carry a `repoKey` captured when the (possibly async) operation starts, so a load resolving after a switch writes to the correct background repo (no-op if closed meanwhile). `openRepo` now switches (preserving tabs/tree/selection) when the repo is already open — which makes the Dock the multi-repo switcher for free. Close (X) removes only the active repo and activates the most recently opened remaining one, else Home. `pinnedRepoKeys` deliberately stays in the dock module (localStorage), not the store — a noted deviation from the plan's sketch. **Four switch-induced regressions found & fixed** (they were impossible before, when changing repo always meant close+reopen): (1) `ChangesProvider` wiped pending changes on repo change — now buckets per repo (`Record<repoKey, WorkingChange[]>`, new storage key `gh-browser-changes-by-repo`; the old keyless-list key is abandoned, not migrated, since it couldn't say which repo it belonged to); (2) `FileView`'s folder Refresh mutated `state.treeCache` directly, which after the refactor only hit the derived projection — switched to `invalidateDir`; (3)+(4) `pulls-store`/`issues-store` kept the previous repo's list across a switch (panel skipped re-fetch because `status === "loaded"`) — both reset on `meta.fullName` change. Also: AddressBar clears its typed input on repo switch. **Acceptance verified via Playwright**: single-repo flows (CRUD, editor, grid) unchanged; second repo opened while repo A kept an expanded tree, an open file tab, and a staged delta in the background; switching back via the Dock chip restored all of it (badge "1" back, correctly absent while in repo B).
- [x] **PLAN.md M12 — Triage app**: `src/features/triage/` builds directly on M7's `usePulls()`/`classifyPr()`. Keyboard navigation (arrow keys or j/k, Space to toggle selection), quick-select-by-class buttons, bulk actions (close ± delete branch, label, request review) each gated by a confirmation summary ("N PRs will be closed, N branches will be deleted") before executing — the plan's explicit acceptance criterion for irreversible bulk actions.
- [x] **PLAN.md M10 — Control Panel**: `src/features/control-panel/` with three applet feature modules (security, access, branch-rules). Scope is detected purely by making the real request and reading the response (403 → "Disabled — missing scope X", 404 → "not available/not enabled") — never by parsing the token — and every applet stays visible with the specific reason rather than silently disappearing. The Access applet's CODEOWNERS generator stages `.github/CODEOWNERS` through the existing M1/M2 changeset (no special write path). Branch Rules is deliberately read-only (a summary of current protection + link to GitHub) — a partial editor over that many interdependent fields would be riskier than useful for v1. **Bug found and fixed while building it**: the Collaborators section first went through a generic lazy-load wrapper whose promise resolved once via an empty-deps `useEffect`, capturing a stale (pre-fetch) value and permanently showing "None found" despite the data having loaded correctly (visible because the CODEOWNERS checkbox list right below it, reading the same state directly, rendered the collaborators correctly). Fixed by rendering Collaborators straight from the parent's already-fetched state instead of routing it through the reusable lazy-load component.
- [x] **PLAN.md M11 — Start menu**: `src/features/start-menu/` with three tabs. Codespaces: `POST /repos/{owner}/{repo}/codespaces` opens the result's `web_url` in a new tab (no embedded VM client, per §11's platform boundary) plus a plain `github.com/codespaces/new?...` deep link; existing codespaces come from `GET /user/codespaces` filtered client-side by `repository.full_name` (no repo-scoped endpoint exists). Releases uses the standard `/releases` endpoint; Packages has no repo-scoped endpoint either, so it probes each common `package_type` for the owner and filters by repo name client-side. **Wiki spike (the plan's required written feasibility note, produced before any editor UI was built)**: a GitHub wiki is a separate git repo (`owner/repo.wiki.git`) with no Contents-API equivalent; GitHub's git smart-HTTP endpoints don't send permissive CORS headers to arbitrary origins (the same reason tools like isomorphic-git require a dedicated CORS proxy), so a pure browser client cannot clone or push one directly. Conclusion: a WYSIWYG wiki editor is not feasible under the current 100%-client-side architecture without a backend proxy (which would really be a preview of Phase 3's ANVIL Core, not browser code) — so only a stub was built (deep link to the wiki on github.com + the feasibility note in the UI), no editor code.
- [x] **PLAN.md M9 — Dock**: `src/features/dock/`, a persistent bar visible whether or not a repo is open. Pinned repos are just a string list in `localStorage`; clicking a chip calls the existing `openRepo(fullName)` (same single-active-repo swap the "Recent" list already did). Aggregate PR/issue badges per pin come from one `GET /search/issues?q=repo:…+is:pr+is:open` call each (returns `total_count` directly — cheaper than paging), deliberately *without* per-PR mergeable/check detail, since fetching that for every pinned repo on a timer is exactly the rate-limit stampede §11 warns about. Polling every 5 minutes, paused on `document.hidden`, plus an immediate poll on any pin-list change and on regaining tab visibility. Rate-limit budget ("4980/5000 requests · resets in 1h") comes from a new tracker in `github.ts` that records the `x-ratelimit-*` headers on every response (previously only read transiently for 403 handling, never stored) — polling backs off below 50 remaining requests. **Two bugs found and fixed while verifying it**: (1) newly-pinned repos didn't get a badge until the next 5-minute tick, because the poll effect only ran once on mount with an empty dependency array — fixed by adding a second effect that polls whenever the pin list changes; (2) the rate-limit display appeared to be completely missing in the first browser test — that was the Playwright mock, not the code: browsers hide all but a small safelist of response headers from JS on cross-origin responses unless the server sends `Access-Control-Expose-Headers`, and the mock wasn't sending it. The real GitHub API demonstrably does (otherwise the pre-existing 403 rate-limit detection in `ghFetch` could never have worked), so this isn't expected to be an issue against the real API.
...
| `src/features/triage/` | **Feature module** (M12): `TriagePanel.tsx` (keyboard nav, bulk actions with confirm), `index.tsx` (`TriageButton`) — consumes M7's `usePulls()` |
| `src/features/control-panel/` | **Feature module** (M10): `ControlPanel.tsx` (tabbed shell), `security/`, `access/`, `branch-rules/` — each own `api.ts` + applet component, `index.tsx` (`ControlPanelButton`) |
| `src/features/start-menu/` | **Feature module** (M11): `releases-api.ts`, `codespaces-api.ts`, `StartMenuPanel.tsx` (tabbed: Releases & Packages / Codespaces / Wiki stub), `index.tsx` (`StartMenuButton`) |
| `src/features/dock/` | **Feature module** (M9): `pins.ts`, `badges-api.ts`, `dock-store.tsx` (`DockProvider`/`useDock`), `Dock.tsx`, `index.tsx` |
...
| 2026-07-15 | PLAN.md M12: Triage app (keyboard-driven bulk PR actions with a confirm-before-irreversible-action summary), built on M7; M8's multi-repo refactor deliberately deferred (see note above M12) |
| 2026-07-15 | PLAN.md M10: Control Panel (Security/Access/Branch-Rules applets), scope detected via real test requests never token parsing; fixed a stale-closure bug in the Collaborators list |
| 2026-07-15 | PLAN.md M11: Start menu (Codespaces launcher, Releases & Packages, Wiki feasibility spike concluding a WYSIWYG editor isn't buildable client-side — stub + deep link built instead) |
| 2026-07-15 | Corrected a wrong claim that M9 (Dock) depends on M8 (maintainer caught it) — implemented PLAN.md M9: Dock with pin/quick-switch, per-repo PR/issue badges, rate-limit budget display, backoff + visibility-aware polling |
| 2026-07-15 | PLAN.md M8 (final milestone): multi-repo store (`repos` map + `activeRepoKey`, legacy-shaped derived view so no consumer changed), per-repo pending-change buckets, switch-safe async dispatches; all 12 Phase-1 milestones now done |
