...
- **M8 deliberately skipped for now**: PLAN.md itself flags the multi-repo workspaces refactor as "the biggest core-state intervention in the whole plan." M1–M7 already built a large amount of code against today's single `state.meta` shape (Explorer, FileView, every feature module) — rushing that rewrite here would risk regressions across all of it without time left to verify as thoroughly as every other milestone.
- **Correction**: an earlier version of this note also claimed M9 (Dock) and M11 (Start menu) were blocked on M8, echoing PLAN.md's own "prerequisite for M9–M11" claim without checking it. The maintainer pushed back, correctly — the Dock's quick-switch just calls the existing `openRepo(fullName)` (same one-repo-at-a-time swap the "Recent" list already does) and its per-pin badge counts are independent background fetches; neither needs simultaneous multi-repo tab/tree state. M11's Codespaces/Releases/Wiki are all scoped to the single currently-open repo too. Both were built in this session after the correction (see their status notes above/below). Only M8 itself — and anything that genuinely needs *several repos open with independent tab/tree state at once* — is actually deferred.
