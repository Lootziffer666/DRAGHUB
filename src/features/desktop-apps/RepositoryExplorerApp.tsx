...
export function RepositoryExplorerApp({ windowId, resource }: WindowContentProps) {
  ...
  // Per-repository hydration status, isolated from every other repo window.
  const req = state.repoRequests[repoKey.toLowerCase()] ?? { loading: false, error: null };
  ...
  if (!repo) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 bg-neutral-950 p-6 text-center">
        {req.error && !req.loading ? (
          <>
            <p className="max-w-md text-sm text-red-300">{req.error}</p>
            <button
